...
const ProductsLists = () => {
  ...
  const sortAtoB = () => setProducts([...Products].sort((a, b) => a.name.localeCompare(b.name)));
  const sortBtoA = () => setProducts([...Products].sort((a, b) => b.name.localeCompare(a.name)));
      ...
      <div className="grid max-lg:grid-cols-2 max-sm:grid-cols-1 grid-cols-3 gap-5 mt-10 justify-items-center">
        {
            products.map((item, index) => (
                <ProductListCard key={index} title={item.name} des={item.description} img={item.imageUrl} link={item.link}/>
            ))
        }
      </div>
    </section>
